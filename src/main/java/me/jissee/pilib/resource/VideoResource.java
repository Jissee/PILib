...
public class VideoResource implements Texture2D {
    ...
    private void playServerSound(ServerLevel lvl, Player pPlayer, Entity pEntity, Holder<SoundEvent> pSound, SoundSource pCategory, float pVolume, float pPitch, long pSeed){
        ...
        lvl.getServer().getPlayerList().broadcast(pPlayer, pEntity.getX(), pEntity.getY(), pEntity.getZ(), (double)pSound.value().getRange(pVolume), lvl.dimension(), new ClientboundSoundEntityPacket(pSound, pCategory, pEntity, pVolume, pPitch, pSeed));
    }
    private void playClientSound(Player pPlayer, Entity pEntity, Holder<SoundEvent> pSound, SoundSource pCategory, float pVolume, float pPitch, long pSeed){
        net.minecraftforge.event.PlayLevelSoundEvent.AtEntity event = net.minecraftforge.event.ForgeEventFactory.onPlaySoundAtEntity(pEntity, pSound, pCategory, pVolume, pPitch);
        if (event.isCanceled() || event.getSound() == null) return;
        pSound = event.getSound();
        pCategory = event.getSource();
        ...
        if (pPlayer == Minecraft.getInstance().player) {
            si = new EntityBoundSoundInstance(pSound.value(), pCategory, pVolume, pPitch, pEntity, pSeed);
        ...
        if(this.entity != null){
            ...
            if(sound != null){
                if(lvl instanceof ServerLevel slvl){
                    playServerSound(slvl,null,entity, Holder.direct(sound),SoundSource.RECORDS,1.0f,1.0f,RandomSource.create().nextLong());
                }else{
                    playClientSound(Minecraft.getInstance().player, entity, Holder.direct(sound),SoundSource.RECORDS,1.0f,1.0f,RandomSource.create().nextLong());
